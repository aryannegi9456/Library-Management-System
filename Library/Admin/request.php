    ...
    <div class="container mt-3">
        <?php
            if (isset($_SESSION['login_user']))
            {
                ...
                <input class="form-control me-2" aria-label="Search" type="text" name="username" placeholder=" Enter Username for approval"
                    required>
                ...
                <?php
                echo '<h3 style="text-align:center; margin-top:10px"><b>Request of Books</b></h3>';
                $sql="SELECT student.username, student_id, Books.Book_id, Books.Name, Authors, Edition, Status FROM student INNER JOIN issue_book ON student.username = issue_book.username INNER JOIN Books ON issue_book.Book_id = Books.Book_id WHERE issue_book.approve = ''";
                $res=mysqli_query($db,$sql);;

                if (mysqli_num_rows($res) == 0) 
                {

                     echo "<h2 style=''><b>Their is no pending request.</b></h2>";
                }
                else 
                {
